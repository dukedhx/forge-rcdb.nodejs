...
class GalleryView extends BaseComponent {
  ...
  componentDidMount(){
    ...
    this.primaryRef.current.scrollTop = 0
  ...
  componentWillMount () {
    ...
    this.settings = this.context.storageSvc.load('gallery')
  ...
  async refresh () {
    ...
    const countRes = await this.context.modelSvc.getCount(
      'gallery', opts)

    const items = await this.context.modelSvc.getModels(
      'gallery', opts)
  ...
  validateAgreement () {
    ...
    return new Promise((resolve) => {
      ...
      const onClose = (result) => {
        ...
        this.context.dialogSvc.off('dialog.close', onClose)
      ...
      this.context.dialogSvc.on('dialog.close', onClose)
  ...
  onInitUpload (data) {
    ...
    const notification = this.context.notifySvc.add({
  ...
  onUploadProgress (data) {
    ...
    const notification =
      this.context.notifySvc.getNotification(data.uploadId)
  ...
  extract (item) {
    ...
    if (!item.spinner) {
      ...
      this.context.extractorSvc.getStatus(itemId).then(() => {

        this.context.extractorSvc.download(itemId)
        ...
        if (error.status === 404) {
          ...
          this.context.socketSvc.getSocketId().then((socketId) => {
            this.context.extractorSvc.extract(itemId, {
              socketId
            })
          })
        }
